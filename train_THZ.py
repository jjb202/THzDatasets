...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            # names = ['Rain100H', 'Rain100L']
            names = ['THZ']
            val_loaders = []

            for name in names:
                dataset_opt['dataroot_gt'] = os.path.join('./datasets/THZ/test', name, 'target')
                dataset_opt['dataroot_lq'] = os.path.join('./datasets/THZ/test', name, 'input')
